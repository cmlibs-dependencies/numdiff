...
static const struct numfmt defaults = {CURRENCY, DP,       THSEP, GROUPING,
                                       POS_SIGN, NEG_SIGN, ECH,   IU};
...
/*
  ...
  *TAIL will point to the character '#'.
  If the string STR does not begin with a valid range of positive integer
  values, then leave *B, *E and *TAIL unchanged and return the error code
  THRLIST_INVALID_FORMAT.

  Remarks:

  1. The following abbreviated range specifications are admitted, where N is a
  positive integer value and M == FIELDNO_MAX :
...
static int set_interval(const char *str, unsigned long *b, unsigned long *e,
                        char **tail) {
  long beg, end;
  char *ptr, *endptr;
  ...
  if ((beg = strtol(str, &endptr, 10)) == 0 || beg > FIELDNO_MAX ||
      beg < -FIELDNO_MAX)
    return THRLIST_INVALID_FORMAT; /* illegal input */
  else if (beg < 0) {
...
static int set_intervals(const char *def, unsigned long *beg1,
                         unsigned long *beg2, unsigned long *end1,
                         unsigned long *end2, int *dbl_rng_spec) {
  char *endptr;
  int rv;
  ...
  if (rv != 0)
    ...
    if ((rv != 0) || (*endptr != '\0'))
      ...
      *dbl_rng_spec = 1;
      return (*end2 + *beg1 == *end1 + *beg2) ? THRLIST_OK
                                              : THRLIST_INVALID_RANGES;
    }
  } else {
    /* *endptr == '\0' */
    *beg2 = *beg1;
...
int thrlist_add(thrlist *plist, const char *def) {
  ...
  if (endptr == def || cmp(r, Zero) < 0) {
    ...
    if (*endptr == '\0') {
      ...
      if ((rv = set_intervals(endptr + 1, &beg1, &beg2, &end1, &end2,
                              &dbl_rng_spec)) != 0) {
        delR(&r);
        return rv;
      }
