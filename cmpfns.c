...
extern void print_lines(const char *line1, const char *line2,
                        ...
                        int delimiter_only);
extern void print_fields(const char *field1, const char *field2, size_t l1,
                         size_t l2, unsigned long lineno1,
                         unsigned long lineno2, unsigned long fieldno1,
...
static void field2cx(const char *field, size_t length, char **tail, int iscale,
                     const struct numfmt *pnf, Complex *pz) {
  char *ptr = (char *)field;
  char ch;
...
static int cmp_fields(const char *field1, const char *field2,
                      unsigned long fieldno1, unsigned long fieldno2, size_t l1,
                      size_t l2, const argslist *argl, statlist *statres,
                      Real *abserr, Real *relerr, int *Labserr_exceeded,
                      int *Rabserr_exceeded) {
  char *tail1, *tail2;
  Complex z1, z2;
      ...
      if (getBitAtPosition(&argl->optmask, _2_MASK) == BIT_OFF)
        exit_code =
            thrlist_cmp(*relerr, argl->maxrelerr, fieldno1, fieldno2) > 0 &&
                    thrlist_cmp(*abserr, argl->maxabserr, fieldno1, fieldno2) >
                        0
                ? 2
                : 0;
      else
...
static int cmp_lines(const char *line1, const char *line2,
                     unsigned long lineno1, unsigned long lineno2,
                     const char **ifs1, const char **ifs2, const argslist *argl,
                     statlist *statres) {
  ...
  if (!line1 && !line2)
    ...
    ilist = argl->ignore;
    while (ilist) {
      if (ilist->regular_expression && match(line1, ilist->regular_expression) == 0) {
        if (match(line2, ilist->regular_expression) == 0) {
            return 0;
        }
      }
      ilist = ilist->next;
 ...
 */
      ...
      if (fieldno1 >= fieldno_upper_limit) {
        printf(_("@ Line %lu in file \"%s\"\n  contains too many fields to be "
                 "properly processed!\n"),
               lineno1, argl->file1);
        exit(EXIT_TROUBLE);
      }
 ...
 */
      ...
      if (*field1 != '\0' && *field2 != '\0') {
        ...
        rv = cmp_fields(field1, field2, fieldno1, fieldno2, l1, l2, argl,
                        statres, &abserr, &relerr, &Labserr_exceeded,
                        &Rabserr_exceeded);
        if ((Labserr_exceeded)) {
        ...
        if (rv >= 1) {
          if ((output_mode > OUTMODE_QUIET) &&
              !(rv == 1 &&
                getBitAtPosition(&argl->optmask, _SE_MASK) == BIT_ON) &&
              !(rv == 2 &&
                getBitAtPosition(&argl->optmask, _SU_MASK) == BIT_ON)) {
            if ((_1sttime)) {
            ...
            print_fields(field1, field2, l1, l2, lineno1, lineno2, fieldno1,
                         fieldno2);
            if (rv == 2)
              ...
              print_separator();
                     ...
                     !(rv == 2 &&
                       getBitAtPosition(&argl->optmask, _SU_MASK) == BIT_ON)) {
            printf("%lu:%lu:%lu:%lu:", lineno1, lineno2, fieldno1 + 1,
                   fieldno2 + 1);
            if (rv == 1) {
              fputs("*:*:", stdout);
            } else /* rv == 2 */
  ...
  */
    ...
    if (*field1 != '\0') {
      if (output_mode >= OUTMODE_NORMAL) {
        ...
        print_fields(field1, field2, 0, 0, lineno1, lineno2, fieldno1,
                     fieldno2);
...
int cmp_files(FILE *pf1, FILE *pf2, const argslist *argl, statlist *statres) {
   ...
   */
    for (n = 0, err1 = err2 = OK; n < table.len; n += 4U) {
      ...
      for (i = 0; i < 4U; i++) {
        ...
        if ((cmp_lines(line1, line2, lineno1, lineno2, (const char **)ifs1,
                       (const char **)ifs2, argl, statres))) {
          files_differ = 1;
        ...
        if (err1 == OUT_OF_MEMORY || err2 == OUT_OF_MEMORY ||
            err1 == FILE_IS_BINARY || err2 == FILE_IS_BINARY ||
            err1 == READING_ERROR || err2 == READING_ERROR)
          goto catch_error;
      }
    }
  } /* end of `if ( (table.ptr) )'*/
  ...
  if (err1 == OK) {
    ...
    case OK:
      if (getc(pf2) != EOF) {
        ...
        fprintf(stderr,
                _("***  File \"%s\" has more lines than file \"%s\",\n"),
