...
/* See cmpfns.c */
extern int cmp_files(FILE *pf1, FILE *pf2, const argslist *argl,
                     statlist *statres);
...
static void load_defaults(argslist *list, statlist *statres) {
  int i;
...
static void init_mpa_support(argslist *list, statlist *statres) {
  ...
  list->ignore = ignorelist_new();
}

static void dismiss_mpa_support(argslist *list, statlist *statres) {
  ...
  ignorelist_dispose(&list->ignore);
  end_mpa();
}
...
static int open_files(const char *name0, const char *name1) {
  register int f;
      ...
      if (fstat(STDIN_FILENO, &files[f].stat) != 0)
        ...
        if (S_ISREG(files[f].stat.st_mode)) {
          ...
          if (pos < 0)
            ...
            files[f].stat.st_size = MAX(0, files[f].stat.st_size - pos);
        }
  ...
  if (status != EXIT_SUCCESS)
    ...
    same_files =
        (files[0].desc != NONEXISTENT && files[1].desc != NONEXISTENT &&
         0 < same_file(&files[0].stat, &files[1].stat) &&
         same_file_attributes(&files[0].stat, &files[1].stat));
    ...
    if (files[1].desc == UNOPENED) {
      ...
      else if ((files[1].desc = open(files[1].name, O_RDONLY, 0)) < 0) {
...
static int compare_files(const argslist *list, int *is_same_physical_file) {
  if ((files[0].desc != NONEXISTENT && files[1].desc != NONEXISTENT &&
       0 < same_file(&files[0].stat, &files[1].stat) &&
       same_file_attributes(&files[0].stat, &files[1].stat))) {
...
static int rewind_files(void) {
  int status = EXIT_SUCCESS;

  if (lseek(files[0].desc, (off_t)0, SEEK_SET) < 0) {
    perror_with_name(files[0].name);
    status = EXIT_TROUBLE;
  }
  if (lseek(files[1].desc, (off_t)0, SEEK_SET) < 0) {
    perror_with_name(files[1].name);
    status = EXIT_TROUBLE;
  }
  return status;
...
static int set_file_pointers(FILE **fpp1, FILE **fpp2) {
  int status = EXIT_SUCCESS;
...
static int isLocationDefined(difference_location loc) {
  return (loc.lineno1 + loc.fieldno1 + loc.lineno2 + loc.fieldno2 > 0 ? 1 : 0);
}

static void print_statistics(const argslist *list, statlist *statres) {
  ...
  if (list->flag > 0) {
    fputs(_("\n  In the computation of the following quantities\n  only the "
            "errors with positive sign are considered:\n"),
          stdout);
    fputs(_("  differences due to numeric fields of the second file that are\n "
            " less than the corresponding fields in the first file are "
            "neglected\n\n"),
          stdout);
  }
  if (list->flag < 0) {
    fputs(_("\n  In the computation of the following quantities\n  only the "
            "errors with negative sign are considered:\n"),
          stdout);
    fputs(_("  differences due to numeric fields of the second file that are\n "
            " greater than the corresponding fields in the first file are "
            "neglected\n\n"),
          stdout);
  }
  if (statres->Ndisperr == 0) {
    if (statres->Nentries == 0)
      ...
      printf(ngettext("\nOne numeric comparison has been done and\nthe "
                      "resulting numeric difference is negligible\n",
                      "\n%d numeric comparisons have been done and\nthe "
                      "resulting numeric differences are all negligible\n",
                      statres->Nentries),
             statres->Nentries);
  } else if (statres->Ndisperr == statres->Nentries) {
    printf(
        ngettext("\nOne numeric comparison has been done and\nhas produced an "
                 "outcome beyond the tolerance threshold\n",
                 "\n%d numeric comparisons have been done, all of them\nhave "
                 "produced an outcome beyond the tolerance threshold\n",
                 statres->Nentries),
        statres->Nentries);
  } else {
    ...
    printf(ngettext("only one numeric comparison has produced an "
  ...
  fputs(_("\nLargest absolute error in the set of the major numerical "
          "differences:\n"),
        stdout);
  ...
  if ((isLocationDefined(statres->Labserr_location))) {
    printf(
        ...
        statres->Labserr_location.lineno1,
        statres->Labserr_location.fieldno1 + 1);
    printf(_("First occurrence (#line, #field) in the second file: %lu, %lu\n"),
           statres->Labserr_location.lineno2,
           statres->Labserr_location.fieldno2 + 1);
  }

  fputs(_("\nLargest relative error in the set of the major numerical "
          "differences:\n"),
        stdout);
  ...
  if ((isLocationDefined(statres->Rabserr_location))) {
    printf(
        _("\nFirst occurrence (#line, #field) in the  first file: %lu, %lu\n"),
        statres->Rabserr_location.lineno1,
        statres->Rabserr_location.fieldno1 + 1);
    printf(_("First occurrence (#line, #field) in the second file: %lu, %lu\n"),
           statres->Rabserr_location.lineno2,
           statres->Rabserr_location.fieldno2 + 1);
  }
...
static void clean_memory(argslist *pList, statlist *pRes) {
  ...
  if ((pHelp) || (pVersion)) {
    ...
    if (test < 0) {
      fputs(_("\n***  The requested comparison cannot be performed:\n"),
            stdout);
      printf(
          _("***  At least one between \"%s\" and \"%s\" is a binary file\n"),
    ...
    if (getBitAtPosition(&list.optmask, _F_MASK) == BIT_OFF && !ident_files) {
