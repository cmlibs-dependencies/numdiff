...
void print_version(const char *progname) {
  printf("%s %s\n", progname, VERSION);
  printf("Copyright (C) 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, "
         "2014, 2015, 2016, 2017  %s <ivprimi@libero.it>\n",
...
void print_help(const char *progname) {
  ...
  printf("%s %s\n", progname,
         "[-s IFS][-D DELIMS][-a THRVAL[:RANGE|:RANGE1:RANGE2]][-r "
         ...
         "2:RANGE][-Z 1:RANGE][-Z 2:RANGE][-m][-H][-f[NUM]][-T][-B][-l "
         "PATH][-o PATH] FILE1 FILE2");
  printf(_("\nCompare putatively similar files line by line and field by "
           "field,\nignoring small numeric differences or/and different "
           "numeric formats.\n\n"));
  printf(_("RANGE, RANGE1 and RANGE2 stay for a positive integer value or\nfor "
           "a range of integer values, like 1-, 3-5 or -7.\n"));
  printf("%s\n%s\n%s\n\n%s\n\n",
         _("The two arguments after the options are the names of the files to "
           "compare."),
         _("The complete paths of the files should be given,\na directory name "
           "is not accepted."),
         _("The given paths cannot refer to the same file\nbut one of them can "
           "be \"-\", which refers to stdin."),
         _("Exit status: 1 if files differ, 0 if they are equal, -1 (255) in "
           "case of error"));
  /* %%% */
  printf(
      "-s, --separators=IFS\n    %s\n    %s\n    %s\n",
      _("Specify the set of characters to use as delimiters\n    while "
        "splitting the input lines into fields"),
  ...
  printf(
      ...
      _("Set to THRVAL the maximum absolute difference permitted\n    before "
        "two numeric fields are regarded as different\n    (The default value "
        "is zero)."),
      _("If a RANGE is given, use the specified\n    threshold only when "
        "comparing fields whose positions lie in RANGE."),
  ...
  printf("-2, --strict\n    %s\n",
         _("Consider two numerical values as equal only if\n    both absolute "
           "and relative difference do not exceed\n    the respective "
           "tolerance threshold"));
  ...
  printf("-P, --positive-differences\n    %s\n",
         _("Ignore all differences due to numeric fields of the second file "
           "that\n    are less than the corresponding numeric fields in the "
           "first file"));
  printf("-N, --negative-differences\n    %s\n",
         _("Ignore all differences due to numeric fields of the second file "
           "that\n    are greater than the corresponding numeric fields in the "
           "first file"));
  ...
  printf("-V, --verbose\n    %s\n",
         _("For every couple of lines which differ in at least one field "
           "print\n    an header to show how these lines appear in the two "
           "compared files"));
  printf("-O, --overview[=NUM]\n    %s\n    %s\n    %s\n    %s\n",
         ...
         _("If \'NUM\' is zero or is not specified, output at most 130 columns "
           "per line."),
         _("If \'NUM\' is a positive number, output at most \'NUM\' columns "
           "per line."),
  ...
  printf("-T, --expand-tabs\n    %s\n",
         _("Expand tabs to spaces in output while displaying the results of "
           "the\n    synchronization procedure (meaningful only together with "
           "option -O or -f)"));
  printf(
      "-B, --binary\n    %s\n",
      _("Treat both files as binary files (only meaningful under Doz/Windoz)"));
  printf("-l, --warnings-to=PATH\n    %s\n",
         _("Redirect warning and error messages from stderr to the indicated "
           "file"));
  ...
  printf(
      "-R, --ignore=RE\n    %s\n",
      _("Ignore changes whose lines all match RE"));
  /* %%% */
...
static int nfset(int opt_ch, const char *opt_arg, argslist *arg_list) {
  if (strlen(opt_arg) <= 2) {
...
static int fselect(const char *str, unsigned char *mask, int mask_size) {
  long beg, end;
  unsigned long n;
  char *ptr, *endptr;
  ...
  if ((beg = strtol(str, &endptr, 10)) == 0 || beg > mask_size ||
      beg < -mask_size)
    ...
    if (*(ptr = endptr + 1) == '\0')
      ...
      if ((end = strtol(ptr, &endptr, 10)) <= 0 || *endptr != '\0' ||
          end > mask_size)
        return -1; /* illegal input */
    }
  }
...
static int is_numfmt_valid(const struct numfmt *pnf) {
  char store[NUMFMT_CHARS];
  char *ptr;
  int i, j;

  for (ptr = pnf->currency;
       *ptr != '\0' && (!is_digit(*ptr)) && *ptr != pnf->dp &&
       *ptr != pnf->thsep && *ptr != pnf->pos_sign && *ptr != pnf->neg_sign;
       ptr++)
    ;
  if (*ptr != '\0')
...
int setargs(int argc, char *argv[], argslist *list) {
  ...
  const char *optstring =
      "h2F:bBVO::qUESIPNz:Z:mHT#:s:D:a:r:R:c:d:t:g:p:n:e:i:f::X:l:o:v";
  struct option long_options[] = {{"help", 0, NULL, 'h'},
                                  {"strict", 0, NULL, '2'},
                                  {"formula", 1, NULL, 'F'},
                                  {"brief", 0, NULL, 'b'},
                                  {"binary", 0, NULL, 'B'},
  ...
  while ((optch = getopt_long(argc, argv, optstring, long_options,
    ...
    case 's':
      ...
      if (((list->ifs1) &&
           !is_string_in_vector(NEWLINE_STR, (const char **)list->ifs1)) ||
          ((list->ifs2) &&
           !is_string_in_vector(NEWLINE_STR, (const char **)list->ifs2))) {
        fprintf(stderr, _("%s: invalid argument after `-%c\' option:\n"),
                PACKAGE, optch);
        fprintf(stderr,
                _("  The list of field delimiters can not be empty and\n  must "
    ...
    case 'D':
      ...
      if (((list->ifs1) &&
           is_char_in_vector(NEWLINE, (const char **)list->ifs1) != 1) ||
          ((list->ifs2) &&
           is_char_in_vector(NEWLINE, (const char **)list->ifs2) != 1)) {
        fprintf(stderr, _("%s: invalid argument after `-%c\' option:\n"),
                PACKAGE, optch);
        fprintf(stderr,
                _("  The list of field delimiters cannot be empty and\n  must "
                  "always include the newline string (\"\\n\").\n  Care that "
                  "the newline character cannot appear\n  in any other "
                  "delimiter than the newline string\n"));
        return -1;
      } else {
        remove_duplicates_from_string_vector(list->ifs1);
        remove_duplicates_from_string_vector(list->ifs2);
    ...
    case 'a':
      ...
      if (rv == THRLIST_INVALID_FORMAT) {
        fprintf(stderr, _("%s: invalid argument after `-%c\' option:\n"),
                PACKAGE, optch);
        fprintf(stderr,
                _("  The format specification has not been respected\n"));
        return -1;
      } else if (rv == THRLIST_INVALID_RANGES) {
        fprintf(stderr, _("%s: invalid argument after `-%c\' option:\n"),
                PACKAGE, optch);
        fprintf(stderr,
                _("  The specified ranges do not have the same length\n"));
        return -1;
      } else
    ...
    case 'r':
      ...
      break;
    case 'R':
      rv = ignorelist_add(&list->ignore, optarg);
      if (rv == IGNORELIST_ERROR) {
        fprintf(stderr, _("%s: invalid argument after `-%c\' option:\n"),
                PACKAGE, optch);
        fprintf(stderr,
                _("  The format specification has not been respected\n"));
        ...
        setBitAtPosition(&list->optmask, _SR_MASK, BIT_ON);
      }
      break;
    case 'c':
      file_id = 0x0;
    ...
    case 'O':
      if (getBitAtPosition(&list->optmask, _F_MASK) == BIT_OFF &&
        ...
        if (!optarg) {
          ...
          /* then set 'w' to 'DEF_ATMOST_NCOLS' and 'suppress_common_lines' to
           * 'TRUE'. */
  ...
  if (getBitAtPosition(&list->optmask, _H_MASK) == BIT_OFF &&
    ...
    fprintf(stderr, _("the following symbols should be all different\nwhile "
                      "two or more of them are actually equal:\n"));
    fprintf(stderr, _("\nDecimal point = `%c\'\n"), list->nf1.dp);
    ...
    fprintf(stderr, _("Symbol used to denote the imaginary unit = `%c\'\n\n"),
            list->nf1.iu);
    return -1;
    ...
    fprintf(stderr, _("the symbol for the leading positive sign (`%c\'),\n"),
            list->nf1.pos_sign);
    fprintf(stderr, _("the symbol for the leading negative sign (`%c\'),\n"),
            list->nf1.neg_sign);
    fprintf(
        stderr,
